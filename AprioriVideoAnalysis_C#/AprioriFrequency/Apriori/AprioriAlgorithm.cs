...
namespace Apriori
{
    class AprioriAlgorithm
    {
        ...
        public static Dictionary<List<int>, int> frequTimeTwo = new Dictionary<List<int>, int>(new ListCompare());  //二层的频率，videoid,assocandidate映射frequency
            ...
            for (int i = 0; i < freOne.Count; i++)
            {
                for (int j = i + 1; j < freOne.Count; j++)
                {
                    ...
                    if (ratioTwo >= minSupThreshold)
                    {
                        ...
                        frequTimeTwo.Add(temp1,togetherNum);
                        frequTimeTwo.Add(temp2, togetherNum);

                        //Console.Write("(" + temp1[0] + "," + temp1[1] + ") "+togetherNum+", ");
                    }
                }
            }
            Console.WriteLine();
